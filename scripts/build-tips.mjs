...
// Navigation tabs markup generator matching existing pages
function getNavigationHtml(activePage = 'tips') {
    const links = [
        { href: '/', label: 'Home', isHome: true, id: 'home' },
        { href: '/projects', label: 'Projects', id: 'projects' },
        { href: '/opensource', label: 'Open Source', isOss: true, id: 'opensource' },
        { href: '/tips', label: 'Tips', id: 'tips' },
        { href: '/resume', label: 'Resume', id: 'resume' },
        { href: '/talks', label: 'Talks', id: 'talks' },
    ];

    const navItems = links.map(link => {
        const isActive = activePage === link.id;
        const activeClass = 'px-2.5 sm:px-3.5 py-2.5 sm:py-3 text-xs sm:text-sm font-medium text-slate-900 dark:text-white border-b-[2.5px] dark:border-b-2 border-red-500 transition-colors inline-flex items-center whitespace-nowrap';
        const inactiveClass = 'px-2.5 sm:px-3.5 py-2.5 sm:py-3 text-xs sm:text-sm font-medium text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white border-b-[2.5px] dark:border-b-2 border-transparent hover:border-slate-300 dark:hover:border-slate-700 transition-colors inline-flex items-center whitespace-nowrap';
        
        let content = link.label;
        if (link.isHome) {
            content = `<span class="site-nav-home-label">Home</span><svg class="site-nav-home-icon icon text-sm" viewBox="0 0 576 512" aria-hidden="true"><path d="M280.37 148.26 96 300.4V464c0 26.5 21.5 48 48 48h96V368c0-8.8 7.2-16 16-16h64c8.8 0 16 7.2 16 16v144h96c26.5 0 48-21.5 48-48V300.3L295.67 148.26c-4.42-3.65-10.88-3.65-15.3 0zM571.6 251.47 488 182.34V44c0-6.63-5.37-12-12-12h-56c-6.63 0-12 5.37-12 12v74.12L318.4 37.6c-17.69-14.61-43.11-14.61-60.8 0L4.4 251.47c-5.1 4.21-5.82 11.77-1.61 16.87l25.6 30.98c4.21 5.1 11.77 5.82 16.87 1.61L280.37 82.26c4.42-3.65 10.88-3.65 15.3 0l235.11 218.67c5.1 4.21 12.66 3.49 16.87-1.61l25.6-30.98c4.21-5.1 3.49-12.66-1.65-16.87z"/></svg>`;
        } else if (link.isOss) {
            content = `<span class="sm:hidden">OSS</span><span class="hidden sm:inline">Open Source</span><svg class="icon text-red-500 text-xs ml-1.5" viewBox="0 0 512 512" aria-hidden="true"><use href="#i-heart"/></svg>`;
        }

        const ariaAttr = isActive ? ' aria-current="page"' : '';
        return `<a href="${link.href}" class="${isActive ? activeClass : inactiveClass}"${ariaAttr}>${content}</a>`;
    }).join('\n                    ');
...
export async function buildTips() {
    ...
    console.log(`✅ Tips build complete: generated tips.html, ${tips.length} individual pages, and search index.`);
}
...
<body class="font-sans m-0 p-0 min-h-screen bg-white dark:bg-slate-950 text-slate-800 dark:text-slate-200 leading-relaxed relative overflow-x-hidden antialiased transition-colors duration-200">
    ...
    <main class="min-h-screen flex flex-col items-center px-3 sm:px-6 pt-1 sm:pt-2 pb-8 sm:pb-12 gap-8">
        <div class="w-full max-w-6xl border border-slate-200 dark:border-slate-800 bg-white/80 dark:bg-slate-900/80 backdrop-blur-md relative z-10">
            ...
            <header class="border-b border-slate-200 dark:border-slate-800 relative z-20">
                <div class="tech-line-h bottom-[-1px]"></div>
                <div class="tech-marker -bottom-[4px] -left-[4px]"></div>
                <div class="tech-marker -bottom-[4px] -right-[4px]"></div>

                <div class="site-nav-row flex items-center overflow-x-auto px-4 sm:px-6">
                    ...
                    <div class="site-nav-utilities flex items-center gap-1 sm:gap-1.5 py-2 pl-2 sm:pl-3 border-l border-slate-200 dark:border-slate-800 shrink-0">
                        <a href="https://github.com/MrPunyapal/tips" target="_blank" rel="noopener noreferrer" aria-label="View Tips repository on GitHub" class="site-nav-source p-1.5 text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white transition-colors flex items-center justify-center rounded" title="GitHub Repository">
            ...
            <section class="border-b border-slate-200 dark:border-slate-800 relative">
                ...
                <div id="empty-state" class="hidden p-12 text-center border-t border-slate-200 dark:border-slate-800">
                    ...
                    <p class="text-xs sm:text-sm text-slate-500 dark:text-slate-400 mb-4">Try adjusting your search query or choosing another category filter.</p>
